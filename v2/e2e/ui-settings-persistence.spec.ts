  ...
  await withClient(async (client) => {
    ...
    const result = await client.query(
      `INSERT INTO users (email, name, provider)
       ...
       RETURNING id`,
      ["e2e-ui-persistence@visor.local", "E2E UI Persistence"],
    );
    ...
    const projectData = {
      ...
      tasks: [
        ...
        },
        {
          ...
          finish: "2026-01-13T00:00:00.000Z",
          duration: 3,
          progress: 20,
          percentComplete: 20,
          isCritical: false,
      ...
      assignments: [],
      budgetItems: [],
      budgetMappings: [{ budgetItemId: "budget-1", taskId: 1, amount: 1000 }],
      baselines: [],
      uiSettings: { locale: "es", taskFilter: { text: "", type: "all" } },
      taskColumnSettings: {
        visible: ["id", "name", "duration", "progress"],
        widths: {},
        labelLocale: "es",
      },
      planningAuditEvents: [],
...
test("persiste y recarga vistas guardadas por rol en project_data", async ({ page }) => {
  ...
  await page.getByTestId("role-view-preset-select").selectOption("executive");
  ...
  await expect
    .poll(async () => (await loadProjectData(projectId))?.uiSettings?.roleViewPreset, {
      timeout: 15_000,
    })
    .toBe("executive");
  ...
  await page.reload();
  await expect(page.getByTestId("gantt-view")).toBeVisible();
  await expect(page.getByTestId("role-view-preset-select")).toHaveValue("executive");
  await expect(page.getByTestId("executive-planning-dashboard")).toBeVisible();

  const persisted = await loadProjectData(projectId);
  expect(persisted.uiSettings).toMatchObject({
    roleViewPreset: "executive",
    taskFilter: { text: "", type: "critical" },
  });
  expect(persisted.taskColumnSettings.visible).toEqual([
    "wbs",
    "name",
    "finish",
    "progress",
