  ...
  await withClient(async (client) => {
    ...
    const result = await client.query(
      `INSERT INTO users (email, name, provider)
       ...
       RETURNING id`,
      ["e2e-what-if@visor.local", "E2E What If"],
    );
    ...
    const projectData = {
      ...
      tasks: [
        ...
        },
        {
          ...
          isSummary: false,
          outlineLevel: 1,
          dependencies: [{ from: 1, to: 2, type: "FS", lag: 0 }],
          wbs: "2",
        },
      ],
      resources: [],
...
function taskDurations(data: { tasks?: PersistedTask[] } | undefined): number[] {
  return (data?.tasks ?? [])
...
test("previsualiza what-if sin mutar y persiste solo al aplicar tras recargar", async ({ page }) => {
  ...
  await page.getByTestId("what-if-duration-delta").fill("2");
  await page.getByTestId("what-if-preview").click();
  await expect(page.getByTestId("what-if-summary")).toContainText("Tareas impactadas");
  expect(taskDurations(await loadProjectData(projectId))).toEqual([1, 1]);

  await page.getByTestId("what-if-discard").click();
  await expect(page.getByTestId("what-if-summary")).toBeHidden();
  expect(taskDurations(await loadProjectData(projectId))).toEqual([1, 1]);
  ...
  await expect
    .poll(async () => taskDurations(await loadProjectData(projectId)), {
      timeout: 15_000,
    })
    .toEqual([3, 1]);
  ...
  await page.reload();
  await expect(page.getByTestId("gantt-view")).toBeVisible();
  await expect(page.locator('[data-testid="gantt-row"][data-task-id="1"]')).toContainText("Actividad base");
  await expect(page.locator('[data-testid="gantt-row"][data-task-id="1"]')).toContainText("3");
  await expect(page.locator('[data-testid="gantt-row"][data-task-id="2"]')).toContainText("Sucesora");
});
